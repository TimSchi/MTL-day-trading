...
##
## Model MT GR optimization on FTSE data
##
...
#define the objective with the graph regularizer
objective = function(W, lambda.g){
...
start_vector = runif(indicators * length(aktien) * window,-5,5) #random initial weights, bounds set after experiments
lambda.g     = c(10^-4,10^-3,10^-2,0.1,1,10,100,10^3,10^4)  #chosen initial lambdas
...
#optimization for all lambdas
out = foreach(i = 1:length(lambda.g))  %dopar%  {
  optim(start_vector, objective, gradient, lambda.g = lambda.g[i], method = "L-BFGS-B")
}
stopCluster(cl)
...
opt.lambda = lambda.g[which.max(profits)]   
...
out1 = optim(runif(indicators * length(aktien) * length(training.x[[1]]),-5,5), objective, gradient, lambda.g = opt.lambda, method = 'L-BFGS-B')
...
TS = foreach (i = 1:131) %dopar% {
  ...
  optim(runif(indicators * length(aktien) * window,-5,5), objective, gradient, lambda.g = opt.lambda, method = 'L-BFGS-B')
}
stopCluster(cl)
