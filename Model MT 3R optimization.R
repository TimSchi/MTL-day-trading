...
##
## Model MT 3R optimization on FTSE data
##
...
#define the objective with three regularizers
objective = function(W,lambda.l,lambda.c,lambda.m){
  
  W = array(W,c(indicators,length(training.x[[1]]),length(aktien)))
  ...
  -sum(sapply(1:length(training.x), function(c) 
    ...
    lambda.l * sum(sapply(1:ncol(training.x[[1]][[1]]), function(i) norm(W[i,,],"F")^2)) +
    lambda.c * sum(sapply(1:length(training.x), function(c) norm(W[,,c] - (1/length(training.x)) * Reduce("+", lapply(1:length(training.x), function(c) W[,,c])))^2))
}

#define the gradient 
gradient = function(W,lambda.l,lambda.c,lambda.m){
    ...
    sapply(1:length(training.x[[c]]), function(m)
      sapply(1:ncol(training.x[[c]][[m]]), function(i) 
        -sum(sapply(1:nrow(training.x[[c]][[m]]), function(d) 
          (cosh(training.x[[c]][[m]][d,] %*% W[,m,c])^-2) * 
            training.y[[c]][[m]][d] * training.x[[c]][[m]][d,i]),na.rm = T) +
        2 * lambda.c * (W[i,m,c] - 1/length(training.x) * sum(sapply(1:length(training.x), function(cs) W[i,m,cs]))) +  
        2 * lambda.l * W[i,m,c] +
...
start_vector = rep(1,indicators * length(aktien) * window)
lambda.c     = c(10^-4,10^-3,10^-2,10^-1,1,10,100,1000,10000)
lambda.m     = c(10^-4,10^-3,10^-2,10^-1,1,10,100,1000,10000)
lambda.l     = c(10^-4,10^-3,10^-2,10^-1,1,10,100,1000,10000)
grid         = expand.grid(ll = lambda.l, lc = lambda.c, lm = lambda.m)
...
out = foreach(i = 1:nrow(grid))  %dopar%  {
  optim(start_vector, objective, gradient, lambda.l = grid$ll[i], lambda.c = grid$lc[i], lambda.m = grid$lm[i], method = "L-BFGS-B")
}
stopCluster(cl)
...
out1 = optim(rep(1, indicators * length(aktien) * length(training.x[[1]])), objective, gradient, lambda.l = opt.lambda$ll, lambda.c = opt.lambda$lc,lambda.m = opt.lambda$lm, method = "L-BFGS-B")
...
TS = foreach (i = 1:131) %dopar% {
  ...
  optim(start_vector, objective, gradient, lambda.l = opt.lambda$ll, lambda.c = opt.lambda$lc, lambda.m = opt.lambda$lm, method = 'L-BFGS-B')
}
stopCluster(cl)
